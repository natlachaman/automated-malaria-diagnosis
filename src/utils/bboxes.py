...
def bboxes_sort(scores, bboxes, top_k=400, scope=None):
    ...
    with tf.name_scope(scope, 'bboxes_sort', [scores, bboxes]):
        # Sort scores...
        scores, idxes = tf.nn.top_k(scores, k=int(top_k), sorted=True)
...
def bboxes_jaccard(bbox_ref, bboxes, name=None):
    ...
    with tf.name_scope(name, 'bboxes_jaccard'):
        ...
        jaccard = tfe_tensors.safe_div(inter_vol, union_vol, 'jaccard')
        return jaccard
...
def bboxes_intersection(bbox_ref, bboxes, name=None):
    ...
    with tf.name_scope(name, 'bboxes_intersection'):
        ...
        scores = tfe_tensors.safe_div(inter_vol, bboxes_vol, 'intersection')
        return scores
...
# =========================================================================== #
def np_bboxes_sort(classes, scores, bboxes, top_k=400):
    """Sort bounding boxes by decreasing order and keep only the top_k
    """
    rclasses, rscores, rbboxes = [], [], []
    for c, s, b in zip(classes, scores, bboxes):
        cc, ss, bb = [], [], []
        for ci, si, bi in zip(c, s, b):
            idxes = np.argsort(-si)
            cc.append(ci[idxes][:top_k])
            ss.append(si[idxes][:top_k])
            bb.append(bi[idxes][:top_k])
        rclasses.append(cc)
        rscores.append(ss)
        rbboxes.append(bb)
...
def np_bboxes_clip(bbox_ref, bboxes):
...
def np_bboxes_jaccard(bboxes1, bboxes2):
...
def np_bboxes_intersection(bboxes_ref, bboxes2):
    """Computing jaccard index between bboxes1 and bboxes2.
    Note: bboxes1 and bboxes2 can be multi-dimensional, but should broacastable.
    """
...
def np_bboxes_nms(classes, scores, bboxes, nms_threshold=0.45):
    """Apply non-maximum selection to bounding boxes.
    """

    rclasses, rscores, rbboxes = [], [], []
    for c, s, b in zip(classes, scores, bboxes):
        keep_bboxes = np.ones(s.shape, dtype=np.bool)
        for i in range(s.size-1):
            if keep_bboxes[i].any():
                # Computer overlap with bboxes which are following.
                overlap = np_bboxes_jaccard(b[i], b[(i+1):])
                # Overlap threshold for keeping + checking part of the same class
                keep_overlap = np.logical_or(overlap < nms_threshold, c[(i+1):] != c[i])
                keep_bboxes[(i+1):] = np.logical_and(keep_bboxes[(i+1):], keep_overlap)

        idxes = np.where(keep_bboxes)
        rclasses.append(c[idxes])
        rscores.append(s[idxes])
        rbboxes.append(b[idxes])
